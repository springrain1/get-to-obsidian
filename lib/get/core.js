...
class GetCore {
    ...
    newMemosCount = 0;
    getTarget;
    copiedAttachments = []; // 已成功复制的附件列表
    // New properties for Get笔记
    notes;
    allTags;
    constructor(notesData, syncedMemoIds = [], getTarget = 'get', copiedAttachments = []) {
     ...
     * @returns boolean
     */
    isAttachmentCopied(attachmentPath) {
    ...
    loadNotes(notesData) {
        const td = new turndown_1.default({ bulletListMarker: '-', headingStyle: 'atx' });
        ...
        td.addRule('tableRow', {
            ...
            replacement: function (content, node) {
                ...
                if (isHeader) {
                    ...
                    const cells = Array.from(node.childNodes || []).filter((n) => n.nodeName === 'TH' || n.nodeName === 'TD');
