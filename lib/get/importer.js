...
class GetImporter {
     ...
     */
    async copyAttachment(sourcePath, targetPath, ensureDir = false) {
    ...
    async import() {
        ...
        try {
            await (0, decompress_1.default)(this.config["rawDir"], tmpDir);
        }
        finally {
        ...
        if (await fs.exists(filesDir)) {
            try {
                ...
                const filesToProcess = allFiles.filter(f => !f.endsWith('.css') && !f.endsWith('.js'));
        ...
        const flomo = new core_1.GetCore(notesData, syncedMemoIds, getTarget, copiedAttachments);
