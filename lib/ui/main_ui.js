...
class MainUI extends obsidian_1.Modal {
    ...
    async onSubmit() {
        ...
        if (this.selectedFile) {
            ...
            try {
                ...
                new obsidian_1.Notice("ZIP 来源解析失败: " + err.message);
                return;
            }
        }
     ...
     */
    async streamFileToTempPath(file, tempPath) {
     ...
     */
    async resolveZipSourceFromSelectedFile() {
        if (!this.selectedFile) {
            throw new Error('selectedFile 为空');
        }
        const file = this.selectedFile;
        const { GET_CACHE_LOC } = await Promise.resolve().then(() => __importStar(require('../get/const')));
    ...
    onOpen() {
        ...
        const imageCheckbox = (0, common_1.createExpOpt)(advancedSection, "导入图片附件 (jpg, png, gif, webp, heic)");
        imageCheckbox.checked = this.plugin.settings.attachmentImport.image;
        imageCheckbox.onchange = (ev) => {
            this.plugin.settings.attachmentImport.image = ev.currentTarget.checked;
        };
        const audioCheckbox = (0, common_1.createExpOpt)(advancedSection, "导入音频附件 (mp3, m4a, wav)");
        audioCheckbox.checked = this.plugin.settings.attachmentImport.audio;
        audioCheckbox.onchange = (ev) => {
            this.plugin.settings.attachmentImport.audio = ev.currentTarget.checked;
        };
        const videoCheckbox = (0, common_1.createExpOpt)(advancedSection, "导入视频附件 (mp4, mov)");
        videoCheckbox.checked = this.plugin.settings.attachmentImport.video;
        videoCheckbox.onchange = (ev) => {
            this.plugin.settings.attachmentImport.video = ev.currentTarget.checked;
        };
        const documentCheckbox = (0, common_1.createExpOpt)(advancedSection, "导入文档附件 (pdf, doc, docx, txt)");
        documentCheckbox.checked = this.plugin.settings.attachmentImport.document;
        documentCheckbox.onchange = (ev) => {
            this.plugin.settings.attachmentImport.document = ev.currentTarget.checked;
        };
        // 自动同步区域
